...
// generateWrapperScript creates a temporary shell script that:
...
func generateWrapperScript(authDir, tokenPath, claudePath string, claudeArgs []string) (string, error) {
	...
	// NOTE: Must use 'export' to make it available to child processes (claude)
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			...
			if apiKey, ok := secrets[apiKeyKey]; ok {
				...
				wrapperScript, err := generateWrapperScript(authDir, tokenPath, claudePath, claudeArgs)
				if err != nil {
				...
				go func() {
					...
					_ = os.RemoveAll(authDir)
					// Re-raise the signal to exit properly
