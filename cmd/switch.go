...
func generateWrapperScript(authDir, tokenPath, claudePath string, claudeArgs []string) (string, bool, error) {
	...
	if isWindows {
		...
		// Read token from file and set environment variable using set /p (handles special chars)
		scriptContent += fmt.Sprintf("<%q set /p ANTHROPIC_AUTH_TOKEN=\r\n", tokenPath)
		// Remove the token file after reading
		scriptContent += fmt.Sprintf("del %q >nul 2>&1\r\n", tokenPath)
