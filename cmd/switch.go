...
func generateWrapperScript(authDir, tokenPath, claudePath string, claudeArgs []string) (string, bool, error) {
	...
	if isWindows {
		...
		scriptContent += "REM Generated by kairo - DO NOT EDIT\r\n"
		scriptContent += "REM This script will be automatically deleted after execution\r\n"
		// Read token from file and set environment variable
		// Use a temp variable to avoid issues with special chars in path
		scriptContent += fmt.Sprintf("set \"TOKEN_FILE=%s\"\r\n", tokenPath)
		scriptContent += "for /f \"usebackq delims=\" %%a in (\"%TOKEN_FILE%\") do set ANTHROPIC_AUTH_TOKEN=%%a\r\n"
		// Remove the token file after reading
		scriptContent += fmt.Sprintf("del \"%s\" >nul 2>&1\r\n", tokenPath)
		// Execute claude with the token as environment variable
		scriptContent += fmt.Sprintf("\"%s\"", claudePath)
	...
	// Make the script executable (Unix) or rename to .bat (Windows)
	if isWindows {
		batPath := f.Name() + ".bat"
		if err := os.Rename(f.Name(), batPath); err != nil {
			_ = os.Remove(f.Name())
			return "", false, fmt.Errorf("failed to rename wrapper script: %w", err)
		}
		return batPath, true, nil
	}
		...
		if err != nil {
			...
			if apiKey, ok := secrets[apiKeyKey]; ok {
				...
				if useCmdExe {
					...
					execCmd = execCommand("cmd", "/c", wrapperScript)
				} else {
