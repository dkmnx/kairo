...
func TestGenerateWrapperScript(t *testing.T) {
	...
	t.Run("generates valid script", func(t *testing.T) {
		...
		if isWindows {
			...
			if !strings.Contains(scriptContent, "set /p") {
				t.Errorf("Wrapper script missing set /p command\nScript content:\n%s", scriptContent)
			}
...
`
	...
	wrapperPath, _, err := generateWrapperScript(authDir, tokenPath, childScriptPath, []string{})
	if err != nil {
		t.Fatalf("generateWrapperScript() error = %v", err)
	}
