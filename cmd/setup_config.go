...
func LoadAndDecryptSecrets(ctx context.Context, configDir string) (map[string]string, string, string, error) {
	result, err := LoadSecrets(ctx, configDir)
	if err != nil {
		return nil, result.SecretsPath, result.KeyPath, err
	}

	return result.Secrets, result.SecretsPath, result.KeyPath, nil
}

func ensureConfigDirectory(cliCtx *CLIContext, configDir string) error {
	return EnsureConfigDir(cliCtx, configDir)
}

func loadOrInitializeConfig(cliCtx *CLIContext, configDir string) (*config.Config, error) {
	return LoadConfig(cliCtx, configDir)
}
...
func saveProviderConfiguration(params SaveProviderParams) error {
	...
	return SaveSecrets(params.CLIContext.GetRootCtx(), params.SecretsPath, params.KeyPath, params.Secrets)
}
...
func addAndSaveProvider(
	...
	return AddAndSaveProvider(AddProviderParams{
		CLIContext:   cliCtx,
		ConfigDir:    configDir,
		Cfg:          cfg,
