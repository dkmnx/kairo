...
The wrapper script mechanism is a critical security feature in Kairo's provider execution. It addresses the fundamental challenge of securely passing API credentials to Claude/Qwen Code process without exposing them in process listings or environment variables.
...
```mermaid
flowchart TB
    A[kairo provider args] --> B[Create private temp dir 0700]
    ...
    K --> L[Wrapper sets env var]
    L --> M[Wrapper deletes token file]
    M --> N[Wrapper execs CLI]
...
The wrapper script generation accepts an optional `envVarName` parameter that defaults to `"ANTHROPIC_AUTH_TOKEN"` for Claude. This allows customization for different CLIs (e.g., Qwen uses `ANTHROPIC_API_KEY`).
...
scriptContent = "# Generated by kairo - DO NOT EDIT\r\n"
scriptContent += "# This script will be automatically deleted after execution\r\n"
// quotePowerShell wraps in single quotes and doubles embedded quotes.
// Inside single quotes PowerShell performs no interpolation, so $, ", `,
// &, ;, |, and % are literal and reach the harness unchanged.
scriptContent += fmt.Sprintf("$env:%s = Get-Content -Path %s -Raw\r\n", envVar, quotePowerShell(tokenPath))
scriptContent += fmt.Sprintf("Remove-Item -Path %s -Force\r\n", quotePowerShell(tokenPath))
scriptContent += fmt.Sprintf("& %s", quotePowerShell(cliPath))
for _, arg := range cliArgs {
    scriptContent += fmt.Sprintf(" %s", quotePowerShell(arg))
}
scriptContent += "\r\n"
```
...
- `exec` replaces the wrapper process with CLI (claude or qwen) (no shell leftover)
...
- Private directory removed after CLI exits
...
// or
execCmd := exec.Command("qwen", "--token", token)
execCmd.Run()
```
...
- Requires changes to Claude/Qwen Code
...
1. **Token Never in Process Environment**: The Claude/Qwen process receives the token via environment variable set by the wrapper, but Kairo's own environment never contains it
...
- **Tests**: `internal/wrapper/` (see `TestGenerateWrapperScript_*` variants)
...
- [x] Increased complexity (~200 LOC)
- [x] Requires careful permission handling
- [x] Requires comprehensive testing
...
- New Claude/Qwen Code authentication methods are introduced
