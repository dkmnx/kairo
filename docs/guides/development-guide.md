...
1. Add the provider definition to the embedded catalog in `internal/providers/catalog.json`:
...
1. Add the provider key to `providerPriority` in `internal/providers/registry.go` so it appears in setup menus.
