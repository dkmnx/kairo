...
// EscapePowerShellArg quotes a string for safe use as a PowerShell argument.
//
// THREAT MODEL: This function protects auth-token and harness-argument
// delivery into a PowerShell subprocess. The escaping is single-quote
// wrapping plus quote doubling — the ONLY transformation PowerShell
// supports inside single quotes. Inside single quotes PowerShell performs
// zero interpolation and zero escape processing: $, `, $( ), @( ), &, ;, |,
// %, and " are all literal, and backslashes are not escape characters. The
// sole escape hatch is a lone quote closing the quoting, so each embedded
// quote is doubled. No other characters are rewritten: rewriting them (e.g.
// backtick-prefixing $) would corrupt the argument value, not harden it.
// Control characters and newlines pass through intact because PowerShell
// single-quoted strings may span multiple lines.
...
func GenerateWindowsScript(envVar string, cfg ScriptConfig) string {
	...
	// Paths and args are single-quoted with ' doubling so $, ", and backticks
	// in them are never interpolated or re-escaped by PowerShell.
	fmt.Fprintf(&sb, "$env:%s = Get-Content -Path %s -Raw\r\n", envVar, EscapePowerShellArg(cfg.TokenPath))
	fmt.Fprintf(&sb, "Remove-Item -Path %s -Force\r\n", EscapePowerShellArg(cfg.TokenPath))
	fmt.Fprintf(&sb, "& %s", EscapePowerShellArg(cfg.CliPath))
