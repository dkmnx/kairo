...
func GenerateWrapperScript(cfg WrapperScriptConfig) (string, bool, error) {
	if cfg.TokenPath == "" {
	...
	if isWindows {
		...
		scriptContent += fmt.Sprintf("& %q", cfg.CliPath)
		...
		scriptContent += "exec " + fmt.Sprintf("%q", cfg.CliPath)
		for _, arg := range cfg.CliArgs {
